...
window.Shell = (function () {
  ...
  /**
   * Landmarks, a skip link and one live region, added once for every page.
   *
   * Every screen is header-plus-body built from divs, so a keyboard user tabs
   * the whole toolbar before reaching the content and a screen-reader user has
   * no way to jump. Rather than restructure twenty pages, the shared chrome
   * labels what is already there and puts a skip link in front of it.
   */
  function landmarks() {
    var bar = document.querySelector('.bar');
    if (bar && !bar.getAttribute('role')) bar.setAttribute('role', 'banner');

    var nav = document.getElementById('nav');
    if (nav) { nav.setAttribute('role', 'navigation'); nav.setAttribute('aria-label', 'Sections'); }

    /* The main region: whatever the page already uses as its body. */
    var main = document.getElementById('main') ||
      document.querySelector('main, .pane, .body, .head');
    if (main) {
    ...
    if (!document.getElementById('shellLive')) {
  ...
  /**
   * Say something to a screen reader. Saves, status changes and errors are all
   * written by replacing innerHTML, which announces nothing at all — so the
   * pages call this alongside whatever they draw.
   */
  function announce(text) {
    var live = document.getElementById('shellLive');
    if (!live) return;
    /* Same string twice in a row is not re-read unless the node changes. */
    live.textContent = '';
    window.setTimeout(function () { live.textContent = String(text || ''); }, 30);
  }
