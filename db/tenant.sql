...
CREATE TABLE leads (
  ...
  ro_id           BIGINT UNSIGNED NULL COMMENT 'the file this lead became, converted or linked',
  ro_link_kind    ENUM('converted','linked') NULL COMMENT 'converted = this lead wrote the file; linked = it already existed',
  ro_linked_at    DATETIME      NULL,
  ro_linked_by    BIGINT UNSIGNED NULL,
  won_by_hand     TINYINT(1)    NOT NULL DEFAULT 0 COMMENT 'won was set at the desk, not by a conversion',
  won_by_user_id  BIGINT UNSIGNED NULL,
  won_by_name     VARCHAR(120)  NULL,
  won_at          DATETIME      NULL,
  win_note        VARCHAR(255)  NULL,
  ...
  -- A file answers to one lead, whether it was converted or linked by hand.
  UNIQUE KEY uq_lead_ro (ro_id),
...
CREATE TABLE appointments (
  id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  kind            ENUM('drop','pickup','return','estimate','appraiser','sublet') NOT NULL,
  ...
  carrier         VARCHAR(120)  NULL COMMENT 'transport company, or the sublet vendor',
  due_back        DATE          NULL COMMENT 'sublet only: when the vendor says it is back',
...
CREATE TABLE ems_imports (
  ...
  customer_phone2 VARCHAR(32)   NULL,
  customer_email  VARCHAR(190)  NULL COMMENT 'CCC 2.01 has no email field; later writers do',
  customer_addr   VARCHAR(190)  NULL,
  customer_city   VARCHAR(96)   NULL,
...
-- What an accepted import overwrote, field by field.
...
INSERT INTO role_caps (role_key, cap_key, can_see, can_change) VALUES
  ...
  ('owner','del_lead',1,1),('owner','win_lead',1,0),('owner','paperwork',1,1),('owner','del_doc',1,1),
