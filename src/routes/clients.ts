...
/**
 * Wholesale accounts are set up by the office, not by whoever writes the file:
 * terms and who gets billed are the books' business. Retail customers and
 * carriers stay with anyone who may edit a repair order.
 */
function mayWrite(caps: { admin: boolean; editRepairOrders: boolean; manageWholesaleClients: boolean },
                  kind: string): boolean {
  if (kind === 'wholesale') return caps.admin || caps.manageWholesaleClients;
  return caps.admin || caps.editRepairOrders;
}
...
export async function registerClients(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/clients', async (req, reply) => {
    ...
    /* The full client list, contact details included — the thing a departing
       manager would want a copy of. Logged hourly per person. Check-in also
       hits this for the wholesale picker, so the entry names the filter. */
    void auditRead(ctx.company!.id, actorFrom(req), {
      entity: 'client_list',
      label: `Client list opened${q.kind ? ' — ' + q.kind : ''}`,
      rows: rows.length
    });
  ...
  app.post('/api/clients', async (req, reply) => {
    ...
    if (!mayWrite(ctx.caps, b.kind)) {
      return reply.code(403).send({
        error: b.kind === 'wholesale'
          ? 'Only accounting and admin can set up wholesale accounts.'
          : 'Not permitted'
      });
    }
  ...
  app.patch('/api/clients/:id', async (req, reply) => {
    ...
    const existing = await tqOne<RowDataPacket & { platform_locked: number; kind: string }>(
      ctx.company!.id, 'SELECT platform_locked, kind FROM clients WHERE id = ?', [id]);
    if (!existing) return reply.code(404).send({ error: 'No such client' });
    if (!mayWrite(ctx.caps, existing.kind)) {
      return reply.code(403).send({
        error: existing.kind === 'wholesale'
          ? 'Only accounting and admin can edit wholesale accounts.'
          : 'Not permitted'
      });
    }
