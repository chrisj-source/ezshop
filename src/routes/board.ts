...
interface BoardRow extends RowDataPacket {
  ...
  close_date: string | null; paid: number;
...
export async function registerBoard(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/board', async (req, reply) => {
    ...
    /* A file is off the board if EITHER stamp is set: `closed_at` is the pickup
       moment, `close_date` is the books date and the flag for "closed properly".
    ...
    const files = rows.map(r => {
      ...
      return scrubMoney({
        ...
        /* The board needs to know a file is closed, not just that it is here:
           the closed view groups by it. */
