...
import {
  DOC_TYPES, extAllowed, extOfKey, extensionOf, isMoneyDoc, readStream, removeFile,
  serveAs, storageKey, writeStream
} from '../lib/storage';
...
export async function registerDocuments(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/ro/:id/documents', async (req, reply) => {
    ...
    for await (const part of req.parts()) {
      ...
      if (!extAllowed(ext)) {
        /* Drain the part or the request hangs, then refuse the whole upload —
           a half-accepted batch is worse than a clear no. */
        part.file.resume();
        return reply.code(415).send({
          error: `Cannot accept a .${ext} file. Photos (JPEG, PNG, HEIC, GIF) and PDFs only.`
        });
      }
  ...
  app.get('/api/documents/:id', async (req, reply) => {
    ...
    /* The type comes off the extension this server assigned at upload, never
       off mime_type — that column holds whatever the client sent. Anything not
       on the preview list goes out as an attachment. */
    const served = wantThumb
      ? { type: 'image/jpeg', disposition: 'inline' as const }
      : serveAs(extOfKey(doc.storage_key), q.download === '1');
    const filename = safeName(doc.label, doc.mime_type);

    reply.header('content-type', served.type);
    reply.header('content-disposition',
      `${served.disposition}; filename="${filename}"; filename*=UTF-8''${encodeURIComponent(filename)}`);
