...
import { audit, auditRead, moneyStr } from '../lib/audit';
...
export async function registerPayroll(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/payroll', async (req, reply) => {
    ...
    /* Everyone's pay, on one screen. Logged — hourly, not per fetch. */
    void auditRead(cid, actorFrom(req), {
      entity: 'payroll', label: `Payroll opened — week ending ${end}`, rows: people.length
    });
  ...
  app.get('/api/payroll/person/:userId', async (req, reply) => {
    ...
    void auditRead(cid, actorFrom(req), {
      entity: 'payroll_person', entityId: userId,
      label: `Pay sheet opened — ${person.name ?? 'staff #' + userId}, week ending ${end}`
    });
