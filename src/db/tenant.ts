...
import { tenantPassword } from '../lib/tenant-credentials';
...
export async function tenantPool(companyId: number): Promise<Pool> {
  ...
  const pool = mysql.createPool({
    ...
    port: loc.db_port,
    user: loc.db_user,
    /* 'DERIVED' computes this shop's own password; anything else resolves a
       named secret. A DERIVED row with no master secret throws here rather
       than falling back to the shared login — deliberately loud. */
    password: tenantPassword(loc),
    database: loc.db_name,
    /* A tenant may be pointed at another host, and the schema has always
       allowed it. Off-box means TLS, whether or not anyone remembered to set
       DB_SSL — otherwise the credential and every customer record cross a
       network in the clear. Loopback keeps the explicit setting only. */
    ssl: config.db.ssl ?? (isLoopback(loc.db_host) ? undefined : { rejectUnauthorized: true }),
