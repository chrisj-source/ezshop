...
export async function provisionCompany(input: ProvisionInput): Promise<ProvisionResult> {
  ...
  try {
    ...
    /* This shop's own MySQL login, scoped to this database and nothing else.
       ...
       not exist. Falls back to the shared login only when no master secret is
       configured — the one place a fallback is right, because refusing to
       create shops on a box mid-migration would be worse than the risk. */
    let secretRef = 'DEFAULT';
    let dbUser = config.db.user;
    if (config.tenantMasterSecret) {
      const login = await grantTenantLogin(admin, {
        company_id: companyId, db_name: dbName, db_host: config.db.host
      });
      secretRef = DERIVED_REF;
      dbUser = login.user;
    }
    ...
    if (companyId) {
      /* The login is named after the company id, so it can be cleaned up even
         if we failed before writing the row that records it. */
      try {
        await dropTenantLogin(admin, { company_id: companyId, db_host: config.db.host });
      } catch { /* never created */ }
