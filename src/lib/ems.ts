...
export function parseEms(files: Array<{ filename: string; buffer: Buffer }>): EmsEstimate {
  ...
  const est: EmsEstimate = {
    ...
    customerPhone: pickStr(ad1, ['OWNR_PH1', 'INSD_PH1']),
    customerPhone2: pickStr(ad1, ['OWNR_PH2', 'INSD_PH2', 'OWNR_PH3', 'OWNR_CELL', 'INSD_CELL']),

    /* CCC 2.01 carries no email field in .ad1 — the reference file set has none.
       Later CCC writers and other estimating systems do, under several names.
       Try them all and keep whatever turns up; a set without one leaves it null
       and the confirm screen simply has nothing to offer. */
    customerEmail: pickStr(ad1, [
      'OWNR_EMAIL', 'OWNR_EM', 'OWNR_EMAL', 'OWNR_E_MAIL',
      'INSD_EMAIL', 'INSD_EM', 'INSD_EMAL', 'EMAIL', 'E_MAIL', 'EMAIL_ADDR'
    ]),
    ...
    insurerPhone: pickStr(ad1, ['INS_CO_PH1', 'INS_CO_PH', 'INS_PH1', 'CARRIER_PH']),
    ...
    adjusterPhone: pickStr(ad1, ['CLM_CT_PH', 'ADJ_PH1', 'ADJ_PHONE', 'CLM_PH1']),
    adjusterEmail: pickStr(ad1, ['CLM_CT_EM', 'CLM_CT_EMAIL', 'ADJ_EMAIL', 'ADJ_EM']),
  ...
  if (!est.customerPhone && !est.customerEmail) {
    warnings.push('The estimate carries no phone or email for the customer — CCC 2.01 files often have neither.');
  }
