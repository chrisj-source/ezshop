...
const SENSITIVE_AREAS: Area[] = ['Money', 'Void / delete', 'Permissions', 'Payroll', 'Access'];
...
/**
 * Record that somebody *looked* at something.
 *
 * The log was write-only in the other sense: it knew every change and nothing
 * about access, so a manager on their last week could page through every closed
 * file and export the client list without leaving a mark. "We can tell you
 * exactly what was accessed" is the first question after an incident.
 *
 * Two rules keep this from drowning the log:
 *
 *  1. Only *bulk* reads. The payroll screen, the client list, a revenue report,
 *     a whole file's document set. Opening one repair order is the job; opening
 *     four hundred is the thing worth seeing.
 *  2. One entry per person, per view, per hour. The board polls, screens
 *     re-fetch on focus, and a row for every poll would make the log unusable
 *     and the disk expensive. An hour is coarse enough to be cheap and fine
 *     enough to show a pattern.
 *
 * Never throws, same as `audit` — nobody loses a page view to a log failure.
 */
export async function auditRead(
  companyId: number,
  actor: Actor,
  view: { entity: string; label: string; entityId?: number | null; rows?: number; detail?: unknown }
): Promise<void> {
  const w: AuditWrite = {
    entity: view.entity,
    entityId: view.entityId ?? null,
    action: 'viewed',
    area: 'Access',
    label: view.label + (view.rows === undefined ? '' : ` — ${view.rows} row${view.rows === 1 ? '' : 's'}`),
    detail: view.detail,
    sensitive: true
  };
     ...
     SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM audit_log a
         WHERE a.user_id = ? AND a.action = 'viewed' AND a.entity = ?
           AND (a.entity_id <=> ?)
           AND a.created_at > DATE_SUB(NOW(), INTERVAL 1 HOUR))`,
    [...values(actor, w), actor.user.id, w.entity, w.entityId]
  ).catch(() => undefined);
}
